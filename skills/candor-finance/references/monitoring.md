...
Use this recipe only after the first full `candor_open({})` briefing has been
...
1. Run the package's workspace pulse operation: `candor_open({"mode":"pulse"})`.
...
4. If `attention` is `present`, run `candor_open({})`, process and triage the
