...
- Start with `candor_get({"operation":"properties.list","args":{"limit":25}})` and follow returned continuations.
- Read the selected property and dated evidence with
  `candor_get({"operation":"properties.get","args":{"property_id":"PROPERTY_ID","limit":100}})`.
- Preview an import with `candor_write({"operation":"properties.preview","input":"<contents of PROPERTY.json>"})`.
...
- Apply with `candor_write({"operation":"properties.apply","args":{"preview_id":"PREVIEW_ID"}})`. Reverse the latest import
  with `candor_write({"operation":"properties.revert","args":{"preview_id":"PREVIEW_ID"}})` when appropriate. A later change
  prevents reversal from overwriting newer evidence; submit a correction instead.
- Use `candor_get({"operation":"net_worth.summary"})` to verify the current contribution.
- Use `candor_write({"operation":"properties.archive","args":{"property_id":"PROPERTY_ID"}})` only to stop tracking. Past
