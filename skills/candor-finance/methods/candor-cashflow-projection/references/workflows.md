...
1. Establish freshness and scope before any arithmetic:
   ...
   candor_get({
     ...
     "reason": "Check balance and recurring freshness before projecting",
     "task_key": "TASK_KEY"
   })
   candor_get({
     "operation": "accounts.list",
     "reason": "Confirm which accounts are in projection scope",
     "task_key": "TASK_KEY"
   })
   candor_get({
...
2. Collect the committed obligations and inflows for the horizon:
   ...
   candor_get({
     ...
     "reason": "Read visible liability balances and utilization",
     "task_key": "TASK_KEY"
   })
   candor_query({
