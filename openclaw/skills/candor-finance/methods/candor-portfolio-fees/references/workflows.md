...
1. Read the investment value series and the per-holding daily records:
   ...
   ```sh
   candor metrics history --metric investment_value --range 3m --reason "Review investment value history" --task-key TASK_KEY
   candor data query holding_valuations --since 2026-06-01 --limit 100 --reason "Inspect daily holding valuations" --task-key TASK_KEY
   candor data query holding_valuations --holding-id HOLDING_ID --limit 100 --reason "Inspect one holding's value history" --task-key TASK_KEY
   candor changes list --domain investments --reason "Review position changes" --task-key TASK_KEY
   ```
