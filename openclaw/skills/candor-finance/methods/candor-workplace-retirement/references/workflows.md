...
1. Establish what the workspace can see:
   ...
   ```sh
   candor coverage get --reason "Check retirement account coverage" --task-key TASK_KEY
   candor accounts list --limit 100 --reason "Find retirement and payroll accounts" --task-key TASK_KEY
   ```
...
2. If a retirement account is connected, read its contribution history. The
   ...
   ```sh
   candor data query investment_transactions --limit 100 --reason "Inventory plan contributions" --task-key TASK_KEY
   candor data query investment_transactions --limit 100 --cursor NEXT_CURSOR --reason "Continue the contribution inventory" --task-key TASK_KEY
   ```
...
4. Cross-check net-pay deposits against the paystub's net figure to anchor
   ...
   ```sh
   candor transactions list --source-account-id SOURCE_ACCOUNT_ID --since START --limit 100 --reason "Anchor the payroll cadence" --task-key TASK_KEY
   ```
...
1. The user changes an election with the employer or provider under their own
   ...
   candor notes create --file NOTE.json --reason "Track an expected deferral change" --task-key TASK_KEY --parent-action ACTION_ID
   ```
...
2. On revisit, recover the due note and its baseline, then capture the new
   ...
   ```sh
   candor notes list --due --reason "Review due retirement follow-up" --task-key TASK_KEY
   candor notes get NOTE_ID --reason "Recover the deferral-change baseline" --task-key TASK_KEY
   ```
...
3. Resolve the note when the posted per-period figures reflect the change;
   ...
   ```sh
   candor notes resolve NOTE_ID --reason "Deferral change verified on payroll evidence" --task-key TASK_KEY --parent-action ACTION_ID
   candor notes update --file NOTE.json --reason "Advance the deferral-change baseline and revisit date" --task-key TASK_KEY --parent-action ACTION_ID
   ```
