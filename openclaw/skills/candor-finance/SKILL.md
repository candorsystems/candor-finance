...
compatibility: Requires Candor CLI 0.3.48 or newer and an authenticated Candor workspace.
metadata:
  author: Candor
  version: "0.1.0"
...
This file is already loaded as OpenClaw's native Candor skill. It is the
operating method; the local `candor` CLI supplies tools and data, not
instructions.
...
Use the `candor` command map above as the operation surface. JSON is
canonical; Markdown is a bounded continuation-friendly projection.
